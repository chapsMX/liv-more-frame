...
export default function LivMore() {
  ...
  const handleConnectGoogle = async () => {
    try {
      ...
      if (data.url) {
        // Pasar el user_fid como state para identificar al usuario después del callback
        const url = new URL(data.url);
        url.searchParams.set('state', context?.user?.fid?.toString() || '');
        
        // Abrir en una nueva ventana
        window.open(url.toString(), 'google_auth', 'width=600,height=600');
      }
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    try {
      const response = await fetch('/api/auth/disconnect', {
        ...
        body: JSON.stringify({ userFid: context?.user?.fid }),
      });
      ...
      if (data.success) {
        ...
        console.error('Error al desconectar:', data.error);
      }
    } catch (error) {
  ...
  // Si el usuario tiene acceso a la app
  ...
  // Si el usuario está en whitelist pero no tiene acceso
  if (isWhitelisted) {
    return (
        ...
        <main className="flex-1 flex items-center justify-center p-2">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className={`text-center ${protoMono.className}`}>
              ...
              <p className="text-gray-400">Access to the beta testing program will be granted soon.</p>
            </div>
  ...
  // Si el usuario no está en whitelist, mostrar la landing page original
  return (
    <div className="min-h-screen bg-black text-white font-mono flex flex-col">
      <main className="flex-1 flex items-center justify-center p-2">
      ...
      {isGoogleConnected ? (
        <div className="text-center">
          <div className="mb-4 text-green-600">
            ...
            <p className="text-sm text-gray-600">
              Conectado el: {formatDate(googleConnection?.updatedAt)}
            </p>
          </div>
          <button
        ...
        <div className="text-center">
          <button
            ...
            className="px-4 py-2 text-sm text-gray-600 border border-gray-600 rounded hover:bg-gray-50 transition-colors"
          >
            Conectar Google Fit
          </button>
        </div>
